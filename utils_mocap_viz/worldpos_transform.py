...
def root_centered_worldpos_path(base_name, dir_csv="extracted_mocap_csv"):
...
def transform_worldpos_to_root_centered(
    ...
    """
    Per-frame root center + frontal yaw in BVH space (Y up, Z forward; no axis remap).
    """
...
def ensure_root_centered_worldpos_csv(
    ...
    **transform_kwargs,
    ...
    """
    Build or load cached root-centered worldpos CSV next to the raw export.

    Returns path to the root-centered CSV.
    """
    if output_csv is None:
        base_name = os.path.basename(source_worldpos_csv).replace("_worldpos.csv", "")
        dir_csv = os.path.dirname(source_worldpos_csv) or "extracted_mocap_csv"
    ...
    if debug:
        print(f"Building root-centered worldpos: {output_csv}")
